...
public class CustomDB extends SQLiteOpenHelper {
    ...
    public void addPhoneNumber(String name, String phone_number){
        ...
        //cv.put(COLUMN_RATING, rating); // "rating" 값을 저장합니다.
    ...
    public void deletePhoneNumber(String name, String phone) {
        SQLiteDatabase db = this.getWritableDatabase();
        String whereClause = CustomDB.COLUMN_NAME + " = ? AND " + CustomDB.COLUMN_PHONE_NUMBER + " = ?";
        String[] whereArgs = {name, phone};
        db.delete(CustomDB.TABLE_NAME, whereClause, whereArgs);
        db.close();
    }

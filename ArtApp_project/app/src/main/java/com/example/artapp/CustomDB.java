...
public class CustomDB extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "art1.db";
    ...
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE " + TABLE_NAME +
                ...
                "rating REAL DEFAULT 0.0);"; // rating 열 추가
        db.execSQL(query);
    }
     ...
     * @param rating      평가 값
     */
    public void addArtwork(String name, String artworkName, float rating) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_NAME, name);
        cv.put(COLUMN_PHONE_NUMBER, artworkName); // 작품명을 저장할 열은 COLUMN_PHONE_NUMBER로 사용
